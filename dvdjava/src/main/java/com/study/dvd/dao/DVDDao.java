...
public class DVDDao {
    ...
    public static int addPublisher(Publisher publisher) {
        ...
        try {
            ...
            successCount = preparedStatement.executeUpdate();   // 쿼리 실행
            generatedSet = preparedStatement.getGeneratedKeys();
            generatedSet.next();
            publisher.setPublisherId(generatedSet.getInt(1));
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            pool.freeConnection(connection, preparedStatement, generatedSet);
        }
